...
export const createCommentService = async (
  ...
  const author = await rpcGetUser<IAuthor>(userId, [
    "_id",
    "name",
    "username",
    "avatar",
...
export const getCommentByPostIdService = async (postId: string) => {
  try {
    const comments = await CommentModel.find({ postId , isDeleted: false });
    ...
    const uniqueUserMentions = Array.from(
      new Set(
        comments
    ...
    const userMentions = await rpcGetUsers<IAuthor[]>(uniqueUserMentions, [
      "_id",
      "name",
      "avatar",
      "username",
    ...
    const commentWithAuthors = comments.map((comment) => {
      const userMention = comment.userMention
        ? userMentionMap.get(comment.userMention) || null
        : null;
...
export const likeOrUnlikeCommentService = async (
  ...
  try {
    ...
    await comment.save();
    const author = await rpcGetUser<Id>(comment.userId, [
      "_id",
      "name",
      "avatar",
      "username",
    ...
    const userMention = comment.userMention
      ? await rpcGetUser<Id>(comment.userMention, ["_id", "username"])
      : null;
    const returnComment = {
      ...comment.toObject(),
      author,
